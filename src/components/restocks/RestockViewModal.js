...
// import { getRestockRequestById, recordArrival, updateShippedQuantity, forceCompleteRequest } from '../../api/restockApi';
// import { useAuth } from '../../hooks/useAuth';
// import { X, CheckCircle, PackagePlus, Edit2, AlertTriangle, History } from 'lucide-react';
...
//             if (firstPending) setArrivalTile(firstPending.tile._id);
//         } catch (err) {
...
//     // --- NEW: Process arrival history from the request data ---
...
//         });
//         return acc;
//     }, []).sort((a, b) => new Date(b.arrivalDate) - new Date(a.arrivalDate)); // Sort by most recent first
//     // -------------------------------------------------------------

//     // (All handler functions like handleRecordArrival, handleEditShippedClick, etc., remain unchanged)
//     const handleRecordArrival = async (e) => { e.preventDefault(); setIsSubmittingArrival(true); try { await recordArrival(requestId, { tileId: arrivalTile, quantity: parseInt(arrivalQty, 10), notes: arrivalNotes, }); onUpdate(); onClose(); } catch (err) { alert(err.response?.data?.message || 'Failed to record arrival.'); } finally { setIsSubmittingArrival(false); } };
//     const handleEditShippedClick = (item) => { setEditingItemId(item._id); setShippedQtyInput(item.quantityShipped); };
//     const handleSaveShipped = async (itemId) => { try { await updateShippedQuantity(requestId, { itemId: itemId, quantityShipped: shippedQtyInput, }); setEditingItemId(null); fetchRequestDetails(); } catch (err) { alert(err.response?.data?.message || 'Failed to update shipped quantity.'); } };
...
//             });
//             setEditingArrival(null); // Exit editing mode
//             fetchRequestDetails(); // Refresh all data
//         } catch (err) {
...
//     if (loading) return <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50"><p className="text-white">Loading Details...</p></div>;
...
//                     {/* --- Center Column: Item Status Table & Arrival History --- */}
...
//                                                         {editingItemId === item._id ? ( <div className="flex items-center justify-center gap-2"><input type="number" value={shippedQtyInput} onChange={(e) => setShippedQtyInput(parseInt(e.target.value, 10) || 0)} className="w-16 p-1 text-center border rounded-md dark:bg-dark-foreground" autoFocus /><button onClick={() => handleSaveShipped(item._id)} className="text-green-500"><CheckCircle size={18}/></button></div> ) : ( <div className="flex items-center justify-center gap-2 group"><span>{item.quantityShipped}</span>{canEditShipped && isRequestMutable && (<button onClick={() => handleEditShippedClick(item)} className="text-gray-400 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity"><Edit2 size={14}/></button>)}</div> )}
//                                                     </td>
...
//                                     <div key={arrival._id} className="p-3 bg-gray-50 dark:bg-dark-background rounded-md">
...
//                                         <p>No arrivals have been recorded for this request yet.</p>
//                                     </div>
//                                 )}
//                             </div>
//                         </div>
...
//                                     <div><label className="font-medium text-sm">Tile *</label><select value={arrivalTile} onChange={e => setArrivalTile(e.target.value)} required className="w-full mt-1 p-2 border rounded-md dark:bg-dark-foreground dark:border-dark-border"><option value="" disabled>Select a tile...</option>{request.requestedItems.map(item => <option key={item.tile._id} value={item.tile._id}>{item.tile.name}</option>)}</select></div>
//                                     <div><label className="font-medium text-sm">Quantity Arrived (Boxes) *</label><input type="number" min="1" value={arrivalQty} onChange={e => setArrivalQty(e.target.value)} required className="w-full mt-1 p-2 border rounded-md dark:bg-dark-foreground dark:border-dark-border" /></div>
//                                     <div><label className="font-medium text-sm">Notes (e.g., Container #)</label><textarea value={arrivalNotes} onChange={e => setArrivalNotes(e.target.value)} rows="3" className="w-full mt-1 p-2 border rounded-md dark:bg-dark-foreground dark:border-dark-border" /></div>
//                                     <div className="flex-grow"></div>
//                                     <button type="submit" disabled={isSubmittingArrival} className="w-full bg-primary text-white font-semibold py-3 rounded-md hover:bg-primary-hover disabled:opacity-50">{isSubmittingArrival ? 'Saving...' : 'Confirm Arrival'}</button>
//                                 </div>
//                             )}
//                         </form>
//                     )}
