// src/components/tiles/BulkUploadModal.js
...
const BulkUploadModal = ({ onClose, onSave }) => {
    ...
    const handleSubmit = async () => {
        ...
        setApiError('');
        try {
            await bulkCreateTiles(parsedData);
            onSave(); // This will refresh the list and close the modal
        } catch (err) {
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-dark-foreground rounded-lg shadow-xl p-6 w-full max-w-4xl max-h-[90vh] flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-2xl font-bold">Bulk Upload Tiles</h2>
                    <button onClick={onClose} className="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-dark-border"><X /></button>
                </div>
                ...
                {parsedData.length === 0 ? (
                    <div className="flex-grow flex flex-col items-center justify-center">
                        <div {...getRootProps()} className={`w-full border-2 border-dashed rounded-lg p-12 text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-primary/10' : 'border-gray-300 dark:border-gray-600 hover:border-primary'}`}>
                            <input {...getInputProps()} />
                    ...
                    <>
                        <h3 className="font-semibold mb-2">Upload Preview</h3>
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-gray-50 dark:bg-dark-background sticky top-0">
                                    <tr>
                                        ...
                                        {requiredHeaders.map(h => <th key={h} className="p-2 text-left font-medium capitalize">{h}</th>)}
                                        {optionalHeaders.map(h => <th key={h} className="p-2 text-left font-medium capitalize">{h}</th>)}
                                    </tr>
                                </thead>
                                <tbody>
                                    {parsedData.map((row, index) => {
                                        ...
                                        return (
                                            <tr key={index} className={`border-t dark:border-dark-border ${hasError ? 'bg-red-50 dark:bg-red-900/20' : ''}`}>
                                                ...
                                                {requiredHeaders.map(h => <td key={h} className="p-2 truncate max-w-[100px]">{row[h]}</td>)}
                                                {optionalHeaders.map(h => <td key={h} className="p-2 truncate max-w-[100px]">{row[h]}</td>)}
                                            </tr>
                                        );
                                    })}
                                </tbody>
                        ...
                        <div className="mt-4 flex justify-between items-center">
                            <button onClick={() => setParsedData([])} className="text-sm text-gray-600 hover:underline">Upload a different file</button>
